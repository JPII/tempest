...
class Graphics3D {
	...
	public int getUserX(int index) {
		return points.get(index).getUserX();
	}

	public int getUserY(int index) {
		return points.get(index).getUserY();
	}

	public int getUserZ(int index) {
		return points.get(index).getUserZ();
	}
	...
	public void setColor(int prismColor) {
		g.setColor(prismColors.get(prismColor));
	}
	...
	public void drawLine(int userX1, int userY1, int userZ1, int userX2,
			int userY2, int userZ2) {
	...
	public int distance(int index1, int index2) {
		double xLength = points.get(index1).getUserX()
				- points.get(index2).getUserX();
		double yLength = points.get(index1).getUserY()
				- points.get(index2).getUserY();
		double zLength = points.get(index1).getUserZ()
				- points.get(index2).getUserZ();
		return (int) (Math.sqrt(xLength * xLength + yLength * yLength + zLength
				* zLength));
	}
	...
	public int makePrism(int userX, int userY, int userZ, int width,
			int height, int depth, Color prismColor) {
	...
	public void fillPrism(int start) {
		...
		Color color2 = new Color((int) (red * 0.95), (int) (green * 0.95),
				(int) (blue * 0.95));
		Color color3 = new Color((int) (red * 0.90), (int) (green * 0.90),
				(int) (blue * 0.90));
		Color color4 = new Color((int) (red * 0.85), (int) (green * 0.85),
				(int) (blue * 0.85));
		Color color5 = new Color((int) (red * 0.80), (int) (green * 0.80),
				(int) (blue * 0.80));
		Color color6 = new Color((int) (red * 0.75), (int) (green * 0.75),
				(int) (blue * 0.75));
		int front = mostForwardIndex(start) - start;

		if (front < 4) {
			Polygon side1 = new Polygon();
			side1.addPoint(points.get(start).getTrueX() + originX,
					points.get(start).getTrueY() + originY);
			side1.addPoint(points.get(start + 1).getTrueX() + originX, points
					.get(start + 1).getTrueY() + originY);
			side1.addPoint(points.get(start + 2).getTrueX() + originX, points
					.get(start + 2).getTrueY() + originY);
			side1.addPoint(points.get(start + 3).getTrueX() + originX, points
					.get(start + 3).getTrueY() + originY);
			g.setColor(color1);
			g.fillPolygon(side1);
		}
		...
		if (front > 3) {
			...
			side6.addPoint(points.get(start + 4).getTrueX() + originX, points
					.get(start + 4).getTrueY() + originY);
			side6.addPoint(points.get(start + 5).getTrueX() + originX, points
					.get(start + 5).getTrueY() + originY);
			side6.addPoint(points.get(start + 6).getTrueX() + originX, points
					.get(start + 6).getTrueY() + originY);
			side6.addPoint(points.get(start + 7).getTrueX() + originX, points
					.get(start + 7).getTrueY() + originY);
			g.setColor(color6);
			g.fillPolygon(side6);
		}
		...
		if (front == 0 || front == 1 || front == 4 || front == 5) {
			...
			side2.addPoint(points.get(start).getTrueX() + originX,
					points.get(start).getTrueY() + originY);
			side2.addPoint(points.get(start + 1).getTrueX() + originX, points
					.get(start + 1).getTrueY() + originY);
			side2.addPoint(points.get(start + 5).getTrueX() + originX, points
					.get(start + 5).getTrueY() + originY);
			side2.addPoint(points.get(start + 4).getTrueX() + originX, points
					.get(start + 4).getTrueY() + originY);
			g.setColor(color2);
			g.fillPolygon(side2);
		}
		...
		if (front == 2 || front == 3 || front == 6 || front == 7) {
			...
			side5.addPoint(points.get(start + 2).getTrueX() + originX, points
					.get(start + 2).getTrueY() + originY);
			side5.addPoint(points.get(start + 3).getTrueX() + originX, points
					.get(start + 3).getTrueY() + originY);
			side5.addPoint(points.get(start + 7).getTrueX() + originX, points
					.get(start + 7).getTrueY() + originY);
			side5.addPoint(points.get(start + 6).getTrueX() + originX, points
					.get(start + 6).getTrueY() + originY);
			g.setColor(color5);
			g.fillPolygon(side5);
		}
		...
		if (front == 0 || front == 3 || front == 4 || front == 7) {
			...
			side3.addPoint(points.get(start).getTrueX() + originX,
					points.get(start).getTrueY() + originY);
			side3.addPoint(points.get(start + 3).getTrueX() + originX, points
					.get(start + 3).getTrueY() + originY);
			side3.addPoint(points.get(start + 7).getTrueX() + originX, points
					.get(start + 7).getTrueY() + originY);
			side3.addPoint(points.get(start + 4).getTrueX() + originX, points
					.get(start + 4).getTrueY() + originY);
			g.setColor(color3);
			g.fillPolygon(side3);
		}
		...
		if (front == 1 || front == 2 || front == 5 || front == 6) {
			...
			side4.addPoint(points.get(start + 1).getTrueX() + originX, points
					.get(start + 1).getTrueY() + originY);
			side4.addPoint(points.get(start + 2).getTrueX() + originX, points
					.get(start + 2).getTrueY() + originY);
			side4.addPoint(points.get(start + 6).getTrueX() + originX, points
					.get(start + 6).getTrueY() + originY);
			side4.addPoint(points.get(start + 5).getTrueX() + originX, points
					.get(start + 5).getTrueY() + originY);
			g.setColor(color4);
			g.fillPolygon(side4);
		}
	...
	public void drawCube(int start) {
		drawPrism(start);
	}

	public void fillCube(int start) {
		fillPrism(start);
	}
	...
	public void rotateXY(double deltaXY) {
		rotateXY(deltaXY, deltaXY);
	}

	public void rotateXZ(double deltaXZ) {
		rotateXZ(deltaXZ, deltaXZ);
	}

	public void rotateYZ(double deltaYZ) {
		rotateXY(deltaYZ, deltaYZ);
	}

	public void rotateXYZ(double deltaXYZ) {
		rotateXYZ(deltaXYZ, deltaXYZ, deltaXYZ);
	}
	...
	private double mostBackwardValue(int start) {
		...
		for (int j = start + 1; j < finish; j++)
			if (points.get(j).getResultantZ() < points.get(smallest)
					.getResultantZ())
				smallest = j;
	...
	private int mostForwardIndex(int start) {
		...
		int largest = start;
		for (int j = start + 1; j < finish; j++)
			if (points.get(j).getResultantZ() > points.get(largest)
					.getResultantZ())
				largest = j;
		return largest;
	}
